...
- Module-level docstrings are enforced for Python and Python-template files
  (`Taskfile.yml`, `scripts/check-module-docstrings.py`).
...
5. Module/file-level docstrings are checked in local hooks and verification tasks
  (`.pre-commit-config.yaml`, `Taskfile.yml`,
  `template/.pre-commit-config.yaml.jinja`, `template/Taskfile.yml.jinja`).
...
- Module-docstring verifier inspects first significant statement in `.py` and
  `.py.jinja` files (`scripts/check-module-docstrings.py`).
