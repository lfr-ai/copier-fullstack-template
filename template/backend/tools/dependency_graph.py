...
def _parse_python_file(py_file: Path) -> ast.AST | None:
    ...
    Args:
        py_file (Path): Python file path.

    Returns:
        ast.AST | None: Parsed AST instance when parsing succeeds.
    """
    try:
...
def _extract_imports(tree: ast.AST) -> list[str]:
    ...
    Returns:
        list[str]: Imported module names.
    """
    imports: list[str] = []
...
def analyze_imports(source_dir: Path) -> dict[str, list[str]]:
    ...
    Returns:
        dict[str, list[str]]: Mapping of module paths to their imports.
    """
