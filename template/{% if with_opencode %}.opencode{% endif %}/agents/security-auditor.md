---
description:
  Performs security audits identifying vulnerabilities, injection risks, data exposure,
...
- Clean architecture Dependency Rule respected (no adapter imports in application
  layer)?

### 6. Profiling Security

- Profiling middleware disabled in production (`PROFILING_ENABLED=false`)?
- `X-Profile-Secret` uses a strong random value (≥ 32 characters)?
- Profile output directory excluded from version control (`.gitignore`)?
- No profiling data (HTML reports, snapshots) in container images?
- Auto-profiling thresholds appropriate (not too sensitive)?
...
- [ ] Profile secret is strong (≥ 32 chars)
```
