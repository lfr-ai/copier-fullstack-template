...
- Hardened `scripts/audit_koda_alignment.py` with explicit expected top-level
  ...
  `koda_automation`
- Updated `scripts/audit_koda_alignment.py` mandatory root targets to include
...
- Renamed root/template jscpd configuration files from `.jscpd.json` to
  `jscpd.json` and updated all commands/checks to match `koda_automation`
  naming parity
- Hardened `scripts/audit_koda_alignment.py` to ignore transient semver-like
